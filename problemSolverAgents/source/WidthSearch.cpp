...
Node* widthSearch(State* initial, State* destination){
    ...
    Action* pdc = nullptr;
    Node * init = new Node(initial, pdc, nullptr, 0);
    Node * current = init;

    if(current->getState()->getName() == destination->getName()){
        cout << current->getState()->getName() << " is the destination.\n" << endl;
        return current;
    }

    queue<Node*>* edge = new queue<Node*>;
    ...
    while((int) edge->size() != 0){
        ...
        for(int i = 0; i < (int) current->getState()->getNeighbors()->size(); i++){
            ...
            State* son = current->getState()->getNeighbors()->at(i);
            cout << "\t son " << i+1 << ": " << son->getName() << endl;
            
            Node* newNode = new Node(son, pdc, current, (current->getCost()+current->getState()->getDistance()->at(i)));
